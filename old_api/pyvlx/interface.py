...
class Interface:
    ...
    async def api_call(
        self, verb, action, params=None, add_authorization_token=True, retry=False
    ):
        """Send api call."""
        ...
        try:
            return await self._api_call_impl(
                verb, action, params, add_authorization_token
            )
        except InvalidToken:
            if not retry and add_authorization_token:
                ...
                return await self.api_call(
                    verb, action, params, add_authorization_token, True
                )
            raise

    async def _api_call_impl(
        self, verb, action, params=None, add_authorization_token=True
    ):
    ...
    async def refresh_token(self):
        ...
        json_response = await self.api_call(
            "auth",
            "login",
        ...
        if "token" not in json_response:
            raise PyVLXException(
                "no element token found in response: {0}".format(
    ...
    def evaluate_response(json_response):
        ...
        if "errors" in json_response and json_response["errors"]:
            ...
            raise PyVLXException(
                "no element result  found in response: {0}".format(
                    json.dumps(json_response)
                )
            )
    ...
    def evaluate_errors(json_response):
        ...
        if (
            "errors" not in json_response
            or not isinstance(json_response["errors"], list)
            or not json_response["errors"]
            or not isinstance(json_response["errors"][0], int)
        ):
            raise PyVLXException(
                "Could not evaluate errors {0}".format(json.dumps(json_response))
            )
